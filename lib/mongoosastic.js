...
Mongoosastic.prototype.connect = function(hosts){
  this.esClient = elasticsearch.Client({
    hosts: hosts
  });
};

Mongoosastic.prototype.plugin = function(schema, options){
  options.esClient = this.esClient;
...
function plugin(schema, options){
  ...
  var esClient  = options && options.esClient ? options.esClient : new elasticsearch.Client({
    host: {
      host: host,
      port: port,
      protocol: protocol,
...
//To allow dependency injection patters and multiple clusters
Mongoosastic.prototype.Mongoosastic = Mongoosastic;

//Singleton
module.exports=new Mongoosastic();