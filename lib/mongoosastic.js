...
Mongoosastic.prototype.connect = function(config){
  console.log("connect!");
...
function plugin(schema, options){
  ...
  console.log("??", options.esClient);
  var esClient  = esClient ? esClient : new elasticsearch.Client({
    host: {
      host: host,
      port: port,
      protocol: protocol,
